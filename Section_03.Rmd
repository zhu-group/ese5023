...
```
Now the file is loaded into the dataset `Keeling_Data`. Here we use the *option* `header = T` so that the first line of `co2_mm_mlo.csv` is loaded as names of the variables. You can try to trun this off by setting `header = F`, and check what happens.
...
You can use `[]` to extract elements of a vector by specifying their coressponding *index*.  
...
Keeling_Data$co2[0]
```
We can minic Keeling Curve using `plot()` function
```{r}
...
```
The above line of script won't work as `Keeling_Data$month` and `Keeling_Data$co2` have different *data types*.
...
When R reads the `co2_mm_mlo.csv` file, it actually loads the table  into something called a `data.frame`, and named it as `Keeling_Data`. A `data.frame` is a structure which R knows how to build out of the basic data types.
...
To better understand data structures behavior, let’s meet the first data structure: the `vector`.
```{r}
...
where -> can be read as are *transformed into*. You can try to force coercion against this flow using the `as.` functions:
...
x
```
Here `LETTERS` is a  a built in vector from `A` to `Z`.
...
A `matrix` is a **two-dimensional** `array` and they are used commonly as part of the mathematical machinery of statistics. We can declare a matrix full of zeros using the `matrix()` function:
```{r}
...
Here `dim()` returns the diminations of the matrix, `nrow()` returns the number of rows of the matrix, and `ncols()` returns the number of columns of the matrix.
...
* Make a matrix contains the numbers 1:50, with 5 columns and 10 rows. Did the matrix function fill your matrix by column, or by row, as its default behaviour? See if you can figure out how to change this. (Hint: read the documentation using `?matrix`)

## Exercise #3

In this exercise, we will take a look at the hourly weather data measured at the Baoan International Airport in the past 10 years. The data set is download from [NOAA Integrated Surface Dataset](https://www.ncei.noaa.gov/metadata/geoportal/rest/metadata/item/gov.noaa.ncdc%3AC00532/html). Suppose you are asked to study how temperature envolves in the past 10 years at this site.

* Download the file [2281305.zip](https://zhu-group.github.io/ese5023/download/2281305.zip), where the namber `2281305` is the site ID.  

* Extract the zip file, you should see a file named `2281305.csv`,  save the `.csv` file to your `working directory`. 

* Load `2281305.csv` file in R, check the variable names with the `names()` function. What do you observe? You can read the comperhensive [user guide](https://www.ncei.noaa.gov/data/global-hourly/doc/isd-format-document.pdf) for variable names and format of the file, only if you want to. 
...
* You may find the temperature data is in a strange format (like `+0160,1`): the first digit is `+` (positive) or `-` (negative), the following four digits stand for temperature of the air degrees Celsius with a scaling factor of `10`, the value after `,` is the quality code where `1` means the data point passed all quality control checks. Be careful with temperature date points as `+9999`, which means the data point is actually missing.
...
* Now handel the missing values. Use `which()` function to check figure out which elements in `BaoAn_T_value2` have a value of `9999`, and replace those with `NA`.
...
<img src="figs/BaoAn_T.png" alt="drawing" width="600"/>

* Try `min()` and `max()` to get minimun and maxium temperature. If you get results showing `NA`, try to add `na.rm=T` option. Finally, try `which.min()` and `which.max()` to get the dates that with minimun and maxium temperature.
