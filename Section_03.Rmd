...
<img src="figs/python_fly.jpg" alt="drawing" width="700"/>
...
The `Spyder` IDE (Integrated Development Environment) is a simple and beginner-friendly integrated development environment for R. It allows you to write, run, and debug your Python code.
...
By default, `Python` runs line by line. For example, if you run the following lines, a triangle would show up in your Console.
...
Or, we can write a few lines with meaningful information:
...
print("but does not like being 35")
```
...
Suppose you want to change the name `Lei` and age `35` in the previous chunk of code, something like:

```{python, eval=F}
# Print a few lines with meaningful information
# Now I change the name and age
print("Once there is a guy Hao, ")
print("he is 25 years old.")
print("He likes the name Hao,")
print("but does not like being 25.")
```
...
# Use replace()
print(note.replace("Environment","Chemistry"))
```
...
Change `AQI1` to `40`, `80`, and `120`, check the output. Change `AQI2` to `40`, `80`, and `120`, check the output. 
...
`Python`, and every package, provide help files for functions. The general syntax to search for help on any function, “function_name”, from a specific function that is in a package loaded into your namespace (your interactive R session): 
...
If you’re having trouble using a function, 9 times out of 10, the answers you are seeking have ***already been answered*** on [Stack Overflow](https://stackoverflow.com/). You can search using the `[python-3.x]` tag.
...
[Hint: you may find `float()` function useful to convert a string to a number]


## Exercise #4

* Create a function named `f2c` to return temperatures in degrees Fahrenheit (*F*) to Celsius (*C*) using the equation:
$$C = (F - 32) * 5/9$$
