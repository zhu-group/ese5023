...
In fact, we should never use data points with the value of `-99.99` (quality=`0`). One good way to do so is to set those values to `NA` (Not Available’ / Missing Values).
...
Here we use `which()` function to find which value is equal to `-99.99`. We `type="l"` to plot a line, you can use `type="p"` to plot points, and `type="o"` to plot points + line.
