...
See here for a very simple example for how to create `Line` and `Polygon` object:
...
Attribute1 <- data.frame(id="River1", use="Flux", water_per_second = 10) 
...
writeOGR(MyPolygons_sp_dfr,dsn = "/data/MyPolygons",
         layer ="Population", driver="ESRI Shapefile")
```

We can also read in spatial data using `readOGR()` (for vector) and `readGDAL()` (for raster/grids) functions from the `rgdal` package. When reading in a shapefile, `readOGR()` requires the following two arguments:
...
Let's use the shape file of China map as an example, [download it](ss). Uncompress it. Open the folder, you will see there are three files of different formats (`.dbf`, `.shp`, `.shx`). In R, we can read in the shape via:
...
In R you can handel the shape file object directly. For example, we can filter provinces with area larger than `100*10000` km^2^ by uisng the `subset()` function:
...
Now the `RasterLayer` is empty, we can add some values to the cells as following. 
...
We can also use `raster()` to read in raster files. Download this [tiff file]() (`wc2.1_10m_wind_11.tif`, ~ 2.4MB), which is the mean wind speed (m s^-1^) in Nov. from 1970-2000 provided by the [WorldClim](https://worldclim.org/data/worldclim21.html). The data is at a resolution of 10 minutes (~ 340 km^2^).
In R, we can load it as:

```{r}
# Read tiff file
Wind_Nov <- raster("/home/lei/Downloads/wc2.1_10m_wind/wc2.1_10m_wind_11.tif")
...
Count how many provenices have an area of between `0.1 million` and `0.2 million` km^2^ in the `China_map` object. Plot those provenices in red.
