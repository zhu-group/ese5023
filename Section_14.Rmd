...
Here `f` is for a floating point format, `E` is for scientific notation. For a `w.d` format, `w `is the width (number of characters) and `d` is the number of decimal places. See this [note](https://pages.mtu.edu/~shene/COURSES/cs201/NOTES/chap05/format.html) for more.
...
Then we can use this module in a main program with `use`:
...
The `-L.` options tells the linker that the libraries can be found in the current directory. `lsubs` is short for `libsubs.a`, and `lconst` is short for `libconst.a`.
...
Write a subroutine `TestLeapYear.f90` to determine whether a year is a leap year. Then `call` this subroutine from a main program (`my.f90`) to check whether `2021` is a leap year. 

## Exercise #4

Write a module `GetDays.f90` to print total number of days in a given year. Then `use` this module in `my.f90` to print total number of days for `2021`.
