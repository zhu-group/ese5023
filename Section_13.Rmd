...
+ `Fortran` is a more verbose language than `R`, `Python`, and `MATLAB`, so it generally will take you much longer to write `Fortran` codes compared to an equivalent code in `Python`. 
...
+ `Fortran` doesn't have a built-in graphics library, so you can't interactively plot results like you can with `Python`. So, with those points in mind, we recommend you stick to `R`, `Python` or `MATLAB` for you day-to-day scientific computing tasks. 
...
Write a `Fortran` script `TestLeapYear.f90` to determine whether the year of `2022` is a leap year.
