...
Now we have some data to work with, and we have learned about the data at the most basic level, our next tasks is to visualize the data. Often, a proper visualization can illuminate features of the data that can inform further analysis. We will look at four methods of visualizing data.
...
T-test is a method of comparing the mean(s). For one sample t-test, the *null hypothesis* is the sample mean equals to a fixed value. For example, let's generate a random sample of size `n = 50` from $N(\mu=0, \sigma^{2}=25)$, and use t-test to check whether the mean is significantlly different from zero.
...
Two sample t-test is almost the same as one sample t-Test, but now the *null hypothesis* changes. Let's generate two random samples, both with a size of `n = 50` from the $N(\mu=0, \sigma^{2}=25)$ distribution, and use t-test to check whether the means are significantlly different.
