...
Let's create a `pandas` series to store AQI (`2021-10-20 11:00`) at Beijing, Shanghai, Guangzhou, and Shenzhen with `city` as the index :
...
#met2.to_csv('Met_2020_new.csv')

# Save to a csv file without index
met2.to_csv('Met_2020_new.csv', index=False)

# Save to a xlsx file 
met2.to_excel('Met_2020_new.xlsx', index=False)

# Save to txt file, separated by tab
met2.to_csv('Met_2020_new.txt', index=False, sep='\t')
```
...
met = met.loc[ (met['WS']!=-999) & (met['WD']!=-999) & 
               (met['T']!=-999) & (met['RH']!=-999) & (met['P']!=-999) & (met['Precip']!=-999) ]
met.head()
```
...
4. Get the date that has the higher ozone.
