...
Let's create a `pandas` series to store AQI (`2025-10-29 15:30`) at Beijing, Shanghai, Guangzhou, and Shenzhen with `city` as the index :

```{python, eval=F}
# AQI of 4 cities on 10/29/2025
...
# Discard Time
met3 = met[['T', 'P', 'WS', 'WD', 'RH', 'Precip', 'WD_new', 'Hour']]
met3.head()
```
...
# Group by WD_new
met3.groupby(['WD_new']).mean()

# Group by followed by sorting
met3.groupby(['WD_new']).mean().sort_values('T', ascending=True)
```

We can also plot data from `groupby`:

```{python, eval=F}
# Group by the observation hour, get hourly means, then plot
met3[['T', 'Hour']].groupby(['Hour']).mean()['T'].plot()
plt.show()
```

Or try to group data by two factors:

```{python, eval=F}
# Group by the observation hour then by wind direction
met3.groupby(['Hour', 'WD_new']).mean()
```
