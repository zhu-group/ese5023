...
TaiYi uses IBM [load sharing facility (LSF)](https://hpc.sustech.edu.cn/ref/lsf_users_guide_v10.1.pdf) for job management. Put is simply, you need to prepare a LSF job file that clearly describe a job, including computational resources, libraries, log files, commands, executable files, and other options. In our case, the LSF (`job.sh`) is written as:
...
* `#BSUB` stands for `bsub` with different opinions as follow:
  ...
  + `-W 00:10` means we ask the compytational resource for `00` hours and `10` minutes.
...
Once the job is submitted, it be assigned with a job ID (e.g., `890825`). Notice that the job may not be run immediately, depending the availability of computational resources. You can check the status of a job with:
