---
title: "Section 04 Version control with Git "
output:
  html_document:
    toc: true
    toc_float:
...
Git tracks the changes you make to files, so you have a record of what has been done, and you can revert to specific versions should you ever need to. Git also makes collaboration easier, allowing changes by multiple people to all be merged into one source. So regardless of whether you write code that only you will see, or work as part of a team, Git will be useful for you. [^2]
...
[^2]: [What Is Git & Why Should You Use It?](https://www.nobledesktop.com/blog/what-is-git-and-why-should-you-use-it#:~:text=Git%20is%20the%20most%20commonly%20used%20version%20control,people%20to%20all%20be%20merged%20into%20one%20source.)

## Why is Git Useful?

* A backup of your files (repository)  [^3]
...
[^3]: [Version Control with Git](https://research.computing.yale.edu/sites/default/files/files/Version%20Control%20with%20Git(1).pdf)

## The Three States

Git has three main states that your files can reside in: **modified**, **staged**, and **committed**: [^2]
...
Pop open the hood of your project and check out the `Activity Logs` located in the footer toolbar of GitKraken. `Activity Logs` provide real time feedback of application and repository-level interactions that occured in GitKraken. `Activity Logs` files are plain text in a standard log file format. Each line displays time of action, action feedback, and performance data measured in milliseconds.

## Integration with GitHub and Push

Now your repository is local, we need to integrate it with GitHub to make a `remote` repository.

* Click `Push`, you will get a message `There is no remote to push to, would you like to add one?`. Select `Yes`.
* Provide your GitHub information and integrate.
...
Provide the URL of the `remote`, then click `Clone the repo`. By doing so, your repository will go back soon.

## Other important usage

For now, we only conver the basic usage of Git. In fact, there are many more important usage of Git, like `branch`, `merge`, `conflict`, and `pull request`. Go to [GitKraken Guide](https://support.gitkraken.com/start-here/guide/) for more.

Git is a powerful command-line tool, check [Version Control with Git](http://swcarpentry.github.io/git-novice/) for more.
...
* Add an R script to this folder, stage and then commit your change.

* Modify the above R script, stage and then commit your change.

* Add another file (e.g., in `.csv` or `.txt` format) to the folder, stage and then commit your change.
...
* Upload a file via GitHub, commit the change.

* Edit your R script online by clicking the file then `Edit this file`, commit the change.
...
* Reopen GitKraken, `clone` the `remote` repository.

## Exercise #3

* Create a local repository `ESE5023_Assignment`

* Push it to GitHub. This is the place you turn in your assignments.

* Go to GitHub, click `ESE5023_Assignment` repository, then `Settings`. Under the `Manage access`, click `Invite a collaborator`, add `MingYang-Hydrate` and `topleo`. By doing so, your TAs will be able to get access to your GitHub `ESE5023_Assignment` repository, they will grade your assignments and provide necessary feedback. 
