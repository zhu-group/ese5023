...
* Upload your `Python` scripts **AND** report to the `ESE5023_Assignments_XXX` repository (where `XXX` is your SUSTech ID) before the due time.
...
Submit your scripts and report via GitHub. The submission time will be determined based on your latest GitHub records.  

***

# 1. Flowchart 

**[5 points]** Write a function `Print_values` with arguments `a`, `b`, and `c` to reflect the following flowchart. Here the red-ish parallelogram operator is to print values in the given order. Report your output with some random `a`, `b`, and `c` values.  
...
**2.2 [5 points]** Write a function `Matrix_multip` to do matrix multiplication, *i.e.*, `M1 * M2`. Here you are **ONLY** allowed to use `for` loop, `*` operator, and `+` operator. 
