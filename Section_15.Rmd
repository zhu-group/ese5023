---
title: "Section 15 Web scraping with `Python` and Class Wrap-up"
output:
  html_document:
    toc: true
    toc_float:
...
+ Download the [chromedriver_win32.zip file](https://chromedriver.chromium.org/downloads), unzip it, and move the folder to your working directory. You may need to use different versions depending on the version of your Google Chrome.
...
In this article, we’ll see how to **automate** this process with `Python`, using the [`requests`](https://docs.python-requests.org/en/latest/), [`BeautifulSoup`](https://www.crummy.com/software/BeautifulSoup/bs4/doc/), and [`Selenium`](https://selenium-python.readthedocs.io/) libraries. `requests` is an elegant and simple HTTP library for `Python`, `BeautisulSoup` is a `Python` library for extracting and parsing data from HTML files, and `Selenium` is used to automate web browser interaction between `Python` and your browser.

## Installing libraries 

Activate the [`cper` environment](https://zhu-group.github.io/ese5023/Section_10.html#Using_a_Python_environment), as we will install `BeautifulSoup` and `Selenium` under it. If you fail to find `cper`, follow [Creating a `Python` environment](https://zhu-group.github.io/ese5023/Section_10.html#Creating_a_Python_environment) to create one.
...
```{python, eval=F}
# URL
data_url = 'https://www.wunderground.com/history/daily/cn/shenzhen/ZGSZ/date/2021-12-21'
...
At this point, you also need to install [ChromeDriver](https://chromedriver.chromium.org/). If using Windows, download the [chromedriver_win32.zip file](https://chromedriver.chromium.org/downloads), unzip it, and move the folder to your working directory. You should have a single `.exe` file called `chromedriver.exe` in a folder called `chromedriver_win32`. Remember the path to this folder and file.

Next, we’re going to write a `my_rendering` function that will run the `ChromeDriver`. This will open a Chrome window, load the web page fully, return the page HTML, and quit the `ChromeDriver`, closing the Chrome window. Be sure to update the path in the `webdriver.Chrome` function to the path where you installed the `ChromeDriver`.
...
Now let's modify the above chunk of code to save data to a local file (`Shenzhen_20211221.csv`).
...
Now that we have our new `scrape_shenzhen_weather` function defined, let’s use it to scrape data for between `2021-01-01` and `2021-01-05`. To do this, we simply need to define the start_date and end_date parameters, and run the function.
...
You should end up with a `.csv` file (`Shenzhen_2021.csv`) containing dated rows. Check it.
...
Modify the `scrape_shenzhen_weather()` function so that it can scrape the information contained in the Daily Observations table from Weather Underground pages for weather observations taken at any airport station within any date range. 
