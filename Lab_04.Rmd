...
**1.** Download the [sst.mnmean.nc](https://psl.noaa.gov/thredds/fileServer/Datasets/noaa.ersst.v5/sst.mnmean.nc) (~ `80 MB`). Read it with `pandas` as the object `ds` 

**2.** Plot mean SST in the latest month. Show a 2-D figure.

**3.** Plot a time series of global monthly mean SST from `1854-01` to `2021-10`.

**4.** It turns out that the previous figure is not correct, since it didn't account for the real area of the `2 degree x2 degree` grids. For this exercise, let's create a `weights` array proportional to the cosine of latitude, as the correct area-weighting factor for data on a regular lat-lon grid. 

**5.** Now use `weights` array to compute and plot the correct global monthly SST `1854-01` to `2021-10`. You results show look like this:

<img src="figs/corrected_SST.png" alt="drawing" width="700"/>  

**6.** Plot the averaged global SST at `June`. Show a 2-D figure.

**7.** Plot the monthly climatology at a point (`114.55 E, 19.15N`) in the South China Sea. Show a 1-D figure.

**8.** Plot the zonal mean climatology. Show a 2-D figure. You results show look like this:

<img src="figs/zonal_mean.png" alt="drawing" width="700"/>  

**9.** Remove the seasonal cycle from `sst`, and plot a timeseries of the anomalies at the point (`114.55 E, 19.15N`).
...
Use the `help()` function to teach yourself how to use `resample()` function. Now resample the anomalies at a frequency of `180` days, replot, and what do you observe?

**11.** Run the following lines:

```{python, eval=F}
# Compute rolling means
ds_anom_rolling = sst_anom.rolling(time=36, center=True).mean()
...
Use the `help()` function to teach yourself how to use `rolling()` function. Now compute the rolling means a frequency of `3` years, replot, and what do you observe?

**12.** Make a timeseries of the anomalies within lat range (`20S`-`20N`) and lon range (`170`E to `170 W`), show resample means and rolling means at the desired frequency. Use the tips we covered in the [last Section](https://zhu-group.github.io/ese5023/Section_08.html), modify the plot as much as you can.
