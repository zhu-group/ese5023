...
In this exercise, we will learn more Linux commands. Use `man` to learn what a command does with different options. 
...
1.10 Use `module avail` to check the installed softwares (modules) on TaiYi. Then choose one to load using `module load`.

# 2. Run an R script on TaiYi

Write a simple R script, name it as `test.R`. Then load `R` using `module load R`.
...
Now print file size (in bytes) of each `*.pdb` file, line by line.
...
4.1 Use `printenv` to print all or the specified environment variables.

4.2 You can set a temporary environment variable like:
...
Notice that the above environment variables are available only for the current session. If you open a new shell, or if you log out, all variables will be lost. Please verify this.
...
Please add the above line to your `.bash_profile`, save `.bash_profile`.

To update your current environment variables, run:

```{bash, eval=F}
source .bash_profile
```

Now we have made `$MY_NAME` a persistent environment variable, so it is loaded every time you log in.  
