...
In this section, we will use the global monthly near surface temperature ([`CESM2_200001-201412.nc`](https://zhu-group.github.io/ese5023/download/CESM2_200001-201412.nc) from the Community Earth System Model (CESM) again for demonstration. To read the data with `xarray`:
...
Making maps is a fundamental part of geoscience research. Maps differ from regular figures in the following principal ways:
...
Let's start with the simple Plate Carrée (French, *for flat square*) projection (a special case of equirectangular projection) with `cartopy`'s `crs` (coordinate reference systems) module. This is typically imported as `ccrs` (Cartopy Coordinate Reference Systems).
...
As you can see, `ccrs.PlateCarree()` creates an axes. We then use `add_feature()` to add land (`LAND` feature) to the axes. Here the pre-defined features are all small-scale (`1:110m`) [Natural Earth datasets](https://www.naturalearthdata.com/):
...
# Add features to the axes
ax.add_feature(cfeature.LAND, edgecolor='black', facecolor='grey', zorder=0)
ax.add_feature(cfeature.RIVERS, edgecolor='red', zorder=1)
...
# Add features to axes using keywords
...
For higher-resolution features, `cartopy` can automatically download and create them from the [Natural Earth Data](https://www.naturalearthdata.com/) or the [GSHHS](https://www.ngdc.noaa.gov/mgg/shorelines/gshhs.html) database.
