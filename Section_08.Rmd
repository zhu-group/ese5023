...
```{python, eval=F}
# This will return the plot object itself (0x22172dfec10) 
# besides drawing the plot.
plt.plot(ozone_data['Ozone'])
```

This draws a line automatically, assuming the values of the X-axis to start from zero going up to as many items in the data. Notice the line `matplotlib.lines.Line2D` in code output, this is because `matplotlib` returns the plot object itself besides drawing the plot. If you only want to see the plot, add one more line:
...
```{python, eval=F}
# Draw multiple plots using for-loops 
...
If you are using `ax` syntax (Object-oriented), you can use `ax.set_xticks()` and `ax.set_xticklabels()` to set the positions and label texts respectively. If you are using the `plt` syntax (Matlab-like), you can set both the positions as well as the label text in one call using the `plt.xticks()`.
