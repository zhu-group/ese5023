...
Finally, you can save the above plot uisng the `ggsave()` function. The default is to save the last plot that you displayed, using the size of the current graphics device. It's smart enough to guesses the type of graphics device from the extension. Let's save the above figure to a local path `D://class` with a name `myplot.png`:
...
gapminder %>%
  ...
  scale_color_discrete(name="Country Name") +
  facet_wrap( ~ continent, nrow=2)
  ggsave("D://class/myplot.png")  # save a stored ggplot
...
We have covered some basics but critical steps of making plots with `ggplot2`. There are many more features that we don't have time to go through in just one section. You can explore the full functionality of `ggplot2()` by studying the resources listed at the end. 
...
Find the plots you want to mimic, edit and customize the scripts.

***

# Image plot

In our field, we plot 2-D maps with values very often. Again, you can learn how to do so with `ggplot2`. But for now, let's learn how to do this using the `image.plot()` function.
...
title(main=paste("Long term (1850-2020) mean surface temperature in Jan."),
...
# Set the png format
png("D://class/Surface_T.png", width=8.5, height=6, units="in", res=400) 
...
Download the monthly anomaly file (`air.mon.anom.nc`, ~ 21 MB) from [Jones (CRU) Air Temperature Anomalies Version 4: CRUTEM4](https://psl.noaa.gov/data/gridded/data.crutem4.html). Plot an image plot to show the surface air temperature monthly anomaly for the most recent month, change the plot title accordingly, and save you file as `Air_monthly_anomaly_recent.png`.
