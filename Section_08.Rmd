...
```{python, eval=F}
# This will return the plot object itself besides drawing the plot.
...
```{python, eval=F}
# Set the figure size
plt.figure(figsize=(8,4)) # 8 is width, 4 is height
...
Previously, I called `plt.plot()` to draw the points. Since there was only **one axes** by default, it drew the points on that axes itself. But now, since we want the points as drawn on different subplots (axes), we have to call the plot function in the respective axes (`ax1` and `ax2` in below code) instead of `plt`.
...
The syntax we’ve seen is the *Object-oriented syntax*, which plot `axes` one after another. Since the original purpose of `matplotlib` was to recreate the plotting facilities of `matlab` in python, the *matlab-like-syntax* relies on a global state and a flat namespace. For example, the above figure can be made by:

```{python, eval=F}
# Create Figure and Subplots
plt.figure(figsize=(8,4), dpi=120) # 8 is width, 4 is height
...
Besides those pre-built styles, the look and feel of various components of a `matplotlib` plot can be set globally using `rcParams`. The complete list of `rcParams` can be viewed by typing:
...
```{python, eval=F}
# Plot using color: red from BASE_COLORS
...
Here, by varying the size and color of points, we create nice-looking bubble plots. Another convenience is you can directly use a `pandas` dataframe to set the `x` and `y` values, provided you specify the source dataframe in the data argument. You can also set the color `c` and size `s` of the points from one of the dataframe columns itself.

Check the links provided at the end for various built-in functions.

## Beyond `matplotlib`

As the charts get more complex, the more the code you’ve got to write. This is the time to consider using a higher level package like `seaborn`, and use one of its pre-built functions. We are not going in-depth into `seaborn`. A lot of `seaborn`’s plots are suitable for data analysis and the library works seamlessly with `pandas` dataframes. `Seaborn` is typically imported as `sns`. Like `matplotlib` it comes with its own set of pre-built styles and palettes. Feel free to go to the [official `seaborn` page](https://seaborn.pydata.org/) for good examples for you to start with.

We have covered the syntax and overall structure of creating `matplotlib` plots, saw how to modify various components of a plot, customized subplots layout, plots styling, colors, palettes, etc. 
...
Make a figure with `6` subplots (panels), and plot the following figures in those subplots one by one:
