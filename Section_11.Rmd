...
<img src="figs/Windows_Vs_Mac_Vs_Linux_13.jpg" alt="drawing" width="700"/>  
...
Log in Taiyi, in your home directory, type:

```{bash, eval=F}
cp /work/ese-ouycc/data_demo .
```
...
Once log in Taiyi, you see a header like `[ese-xxx@login02 ~]$`. Here `ese-xxx` is your username, `login02` is the login in node, and the dollar sign `$` is a **prompt**, which shows us that the shell is waiting for input; your shell may use a different character as a prompt and may add information before the prompt.
...
1.1 In your home directroy, creat a folder `TEST`, and three subfolders `t1`, `t2`, and `t3` inside `TEST`. 

1.2 Change your directory to `t1`, creat a text file `my.txt`, write your name and student ID in it, and save the file.

1.3 Copy `my.txt` in `t1` to `TEST`, and rename it to ``my_new.txt`.

1.4 Delete `t1`, `t2`, and `t3`.

1.5 In `TEST`, type `cat my_new.txt`. What do you see? Use `man cat` to learn more about `cat`, and quite the help document by typing `q`.
...
The `data_demo/north-pacific-gyre/2012-07-03` folder stores some obervations from a cruise campigan. Each file should contain 300 lines of data points. But there is one file contains less lines than the rest of files. Can figure out which one?
