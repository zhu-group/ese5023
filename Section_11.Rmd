...
Once log in to Taiyi, you see a header like `[ese-xxx@login03 ~]$`. Here `ese-xxx` is your username, `login03` is the login in node, and the dollar sign `$` is a *prompt*, which shows us that the shell is waiting for input; your shell may use a different character as a prompt and may add information before the prompt.
